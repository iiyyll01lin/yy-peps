...
W02 = notebook([
    md(
        ...
        "`[x, sin(2^0 pi x), cos(2^0 pi x), ...]`. PEPS reinterprets each frequency as",
        ...
        "(paper Fig. 2) and shows the Identity-encoder equivalence to APE.",
        "",
        "**繁體中文.** 絕對位置編碼(APE)把 `x` 映成",
        "`[x, sin(2^0 pi x), cos(2^0 pi x), ...]`。PEPS 把每個頻率重新詮釋為在",
    ...
    md(
        "## 2. Identity-encoder PEPS == APE / Identity 編碼器的 PEPS 等價於 APE",
...
W03 = notebook([
    ...
    BOOT,
    md("## 1. Setup: one Kodak image / 設定:一張 Kodak 影像"),
    code(
        "from apps.image.data import load_image, image_to_coords_targets, find_kodak",
    ...
    md("## 3. Plot the bottleneck (reproduce Fig. 5) / 畫出瓶頸(重現 Fig.5)"),
    code(
        "import matplotlib.pyplot as plt",
