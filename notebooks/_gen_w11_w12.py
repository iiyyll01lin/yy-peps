...
W12 = nb([
    ...
    md("## 3. Compare RDNA3.5 vs RDNA4 / 對照 RDNA3.5 與 RDNA4",
       ...
       "是第二資料點。用兩次執行填下表。"),
    code("# Paste both boxes' ms/iter here after running on each:",
         "results = {",
         "  'RDNA3.5 (gfx1151, Box A)': None,   # e.g. 0.0092",
         "  'RDNA4   (gfx1201, Box B)': None,",
         "}",
         "for k, v in results.items():",
         "    print(f'{k}: {v if v is not None else \"(run on that box)\"} ms/iter')"),
    md("## 4. Takeaway / 小結",
