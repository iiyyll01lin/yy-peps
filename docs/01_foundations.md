...
in the FFT (lab W01, cells 2–3). Measured example: a plain MLP with no positional
encoding reaches only ~18–20 dB PSNR, versus ~40 dB once we add encoding.

**繁體中文.** 隱式神經表示把訊號存成函式 `f_theta(座標)->數值`。影像即
`f(x,y)->(r,g,b)`。訓練讓網路重現已知像素,網路便連續地表示該影像。純 MLP 有
**頻譜偏差**——先擬合低頻、很晚才擬合高頻的已知傾向。在 Kodak 影像上表現為模糊的
重建與 FFT 中缺失的高頻環(W01 實作 cell 2–3)。實測:無位置編碼的純 MLP 只到約
18–20 dB,加了編碼後可達約 40 dB。
...
transform of APE features — so **PEPS with an identity encoder equals APE** (lab
W02 verifies a <1e-4 least-squares residual). PEPS is therefore a strict
generalization of positional encoding.
...
投影+串接的特徵是 APE 特徵的仿射變換,因此 **identity 編碼器的 PEPS 等於 APE**
(W02 實作驗證最小平方殘差 <1e-4)。故 PEPS 是位置編碼的嚴格泛化。
...
parameters. Lab W03 reproduces the params-vs-PSNR curves: the grid baseline
saturates while Grid-PEPS keeps climbing for the same budget. This is the
motivational centerpiece of the course.
...
——用相同參數榨出更多訊號。W03 實作重現「參數 vs PSNR」曲線:grid baseline 飽和,
相同預算下 Grid-PEPS 持續上升。這是整門課的動機核心。
